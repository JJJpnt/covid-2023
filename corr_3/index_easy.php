...
// On peut utiliser une boucle pour filtrer les éléments du tableau, en ne prenant que ceux qui correspondent à la période de temps et au pays que je veux
$filteredData = [];
foreach($data as $item) {
    $timestampElement = strtotime($item['date']);
    if($item['nom'] === $pays && $timestampElement >= $timestampDebut && $timestampElement <= $timestampFin) {
        $filteredData[] = $item;
    }
}
